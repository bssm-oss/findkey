...
- 결과 행을 더블클릭하면 선택한 탐지 항목의 상세 정보와 원본 리포트를 sheet로 확인 가능
...
> 참고: 현재 릴리즈 앱은 **ad-hoc signed / not notarized** 상태이므로, macOS 환경에 따라 Gatekeeper 경고가 나타날 수 있습니다.

### 로컬에서 직접 실행하기

개발 또는 수정 중인 코드를 바로 실행하려면 먼저 스캐너를 설치해야 합니다.
...
이 테스트는 UI 자동화가 아니라, URL 파싱 / GitHub owner resolution / finding parser 동작 같은 핵심 로직을 반복 가능하게 검증합니다. 실제 스캐너 설치 상태나 실 저장소 스캔 성공까지 보장하는 테스트는 아닙니다.

## 사용 방법

1. 앱을 실행합니다.
2. 다음 형식 중 하나의 GitHub URL을 입력합니다.
   - `https://github.com/orgs/<org>/repositories`
   - `https://github.com/<user>?tab=repositories`
   - `https://github.com/<owner>`
3. private 저장소를 보거나 GitHub API rate limit을 줄이고 싶다면 토큰을 입력합니다.
4. `저장소 조회`를 눌러 스캔 대상을 불러옵니다.
5. `스캔 시작`을 눌러 Gitleaks + TruffleHog 검사를 실행합니다.
6. 결과 표에서 항목을 확인하고, 필요한 경우 결과를 더블클릭해 상세 창에서 탐지 내용과 원본 리포트를 확인합니다.

정상 동작 시에는 저장소 목록이 조회되고, 스캔 후 결과 수와 원본 리포트를 같은 앱 안에서 확인할 수 있습니다.
...
- private 저장소까지 검사하려면 토큰 입력이 사실상 필요합니다.
- public 저장소만 보더라도 rate limit 회피를 위해 토큰 입력이 유용할 수 있습니다.
...
6. raw JSON/NDJSON을 raw report 뷰와 결과 상세 sheet에서 표시
...
- 사용자 요청이 있을 때는 선택한 탐지 항목의 상세 값과 원본 리포트를 UI에서 직접 확인할 수 있게 유지하기
- 문서와 실제 동작을 항상 맞추기
